    ...
    <?php
     ...
     /*   $url = "dijkstra.ug.bcc.bilkent.edu.tr" ;
        ...
        $username = "HaruChan";
           ...
           /* $result = $conn->query("SELECT * FROM (SELECT DISTINCT * FROM (SELECT DISTINCT * FROM scientific_research_paper S NATURAL JOIN author_has_paper A WHERE username = 'HaruChan' LEFT OUTER JOIN (SELECT paperID, SUM(isViewed) AS tot_view, SUM(isLiked) AS tot_like, SUM(isDownloaded) AS tot_download FROM subscriber_likes_downloads_views_paper GROUP BY paperID) R  ON (R.paperID = T.paperID)) X  LEFT OUTER JOIN (SELECT paper_to_be_cited, COUNT(*) AS cite_no FROM paper_citation GROUP BY paper_to_be_cited) Y   ON (Y.paper_to_be_cited = X.paperID)");
                            ...
                            while($row = $result->fetch_assoc()){
                                echo "<div class='row-fluid'>";
                                     echo "<div class='col-10'>";
                                        echo "<p>" . "Title: " . $row['title'] . " ". "Author: " . $row['username'] . " " . "View: " . $row['tot_view'] . 
                                            "       " . "Like: " . $row['tot_like'] . 
                            ...
                            while($data = $sql->fetch_assoc() )
                            {
                                while($data2 = $sql2->fetch_assoc())
                                {
                                    if($data['paperID'] == $data2['paperID'] )
                                    {
                                        echo "<div class='row-fluid'>";
                                            echo "<div class='col-10'>";
                                                echo "<p>" . "Title: " . $data['title'] . " " . " Language: " . $data['language'] . " ". " Page Number: " . $data['page_number'] . " " . " Like: " . $data2['COUNT(isLiked)'] . " Download: " . $data2['COUNT(isDownloaded)'] . " " . "Viewed: " . $data2['COUNT(isViewed)'] . "</p>" ;
                                            echo "</div>";
                                        echo "</div>";
                                    }
                                }
