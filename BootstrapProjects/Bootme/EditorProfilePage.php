		...
		<?php
				...
				echo "<form action='EditorProfilePage.php' method='POST'>";	
					...
					echo "<div class='form-row pt-5'>";
						echo "<div class='col-2'>";
							...
							echo "<input class='btn-lg btn-primary my-2' type='submit' name='AssignRev' value='Assign Reviewer'>";
			...
			if($_SERVER['REQUEST_METHOD'] === 'POST'){
				if(isset($_POST['DeleteAccount'])){
					...
					$result4 =  $conn->query("DELETE FROM conference_editor WHERE username='$user_username'");
					$result5 =  $conn->query("DELETE FROM subscriber WHERE username='$user_username'");
					if($result1 || $result2 || $result3 || $result4 || $result5 ){
						$actual_link = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
						$togo = substr($actual_link, 0, strpos($actual_link, '/EditorProfilePage.php'));
						$togo = $togo . "/LoginPage.php";
						header("Location: $togo");
					}
				}
				if(isset($_POST['AssignRev'])){
					$actual_link = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
					$togo = substr($actual_link, 0, strpos($actual_link, '/EditorProfilePage.php'));
					$togo = $togo . "/AssignReviewerPage.php";
